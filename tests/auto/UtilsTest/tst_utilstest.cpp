...
void UtilsTest::test_parseValuesList2() {
...
void UtilsTest::test_expandEnvironmentStrings() {
#ifdef Q_OS_WIN
...
#ifdef Q_OS_WIN
void UtilsTest::test_getWindowsApiErrorMessage() {
...
void UtilsTest::initTestCase() {
  ...
  if (retVal) {
    QString tstPath =
        m_basePath + QStringLiteral("/") + m_newDir + QStringLiteral("/file");
...
void UtilsTest::test_dataSourceListCount() {
  QString tstPath = m_basePath + QStringLiteral("/") + m_newDir;
  QStringList retVal =
      elcUtils::getDataSourceList(tstPath, QStringList() << "*.csv");
  QCOMPARE(retVal.size(), filesCount);
}

void UtilsTest::test_sanitizeValue() {
  QVERIFY(elcUtils::sanitizeValue(QLatin1String("^([0-9.]+)$"), "10.10.10.10"));
  QVERIFY(
      !elcUtils::sanitizeValue(QLatin1String("^([0-9.]+)$"), "l0.10.10.10"));
...
void UtilsTest::test_getMetaClassInfo() {
