...
CBasicDatabaseTest::CBasicDatabaseTest(QObject *parent) : QObject{parent} {}

CBasicDatabaseTest::~CBasicDatabaseTest() { m_fakeDB.close(); }
...
void CBasicDatabaseTest::test_initDB() {
  bool retVal =
      m_fakeDB.init(QLatin1String("QSQLITE"), SRCDIR "data/fakedb.db");
  QVERIFY(retVal);
}
...
void CBasicDatabaseTest::test_getValues() {
  ...
  while (m_fakeDB.isNext()) {
    ...
    QCOMPARE(m_fakeDB.geValue(1).toString(),
             QString("2023-05-09T11:19:57.36Z"));
    QCOMPARE(m_fakeDB.geValue(2).toString(), QString("edbfa4ea24038861"));

    QCOMPARE(m_fakeDB.geValue(4).toString(),
             QString("username: mr_data,\r\n  type: PASSWORD,\r\n  ip address: "
    ...
    QCOMPARE(m_fakeDB.geValue(9).toString(),
...
void CBasicDatabaseTest::test_truncateTable() {
  ...
  QVERIFY(retVal);
  dataList_t res = m_fakeDB.findInDB(
