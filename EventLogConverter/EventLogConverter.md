...
**Примечание.** Если в файлах будут дубликаты записей (к примеру, из-за наложения диапазонов дат выборок), то утилиты отбросят дубликаты. Это особенно актуально для журналов аудита системы (Audit Tail и System log), т.к. временные метки имеют точность до секунды и одинаковые события произошедшие в течении секунды, будут записаны в базу как одно событие.  
