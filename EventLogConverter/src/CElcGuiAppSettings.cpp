...
inline const QVariant nullStringValue =
    QVariant(QMetaType::fromType<QString>()); // Qt 6 only
inline const QVariant nullDateTimeValue =
...
CElcGuiAppSettings &CElcGuiAppSettings::instance() { return g_elcGuiSettings; }

void CElcGuiAppSettings::createDefault(const QString &iniPath) {
...
#ifdef QT_DEBUG
  settings.setValue(QLatin1String("default_monitor"),
                    1); // "\\.\DISPLAY1" in the my hardware config
#else
  settings.setValue(QLatin1String("default_monitor"),
                    0); // 0-auto, 1-first, 2-second, etc.
#endif
  settings.endGroup();
#endif
}

QString CElcGuiAppSettings::getLastDir() const {
  return getMain(QLatin1String("HISTORY/last_dir")).toString().trimmed();
}
#ifdef Q_OS_WIN
quint32 CElcGuiAppSettings::getDefaultMonitor() const {
