...
void initTranslation(QTranslator *translator, const QApplication *qa,
                     const QStringList &list, const QString &fileName) {
#ifdef Q_OS_LINUX
  ...
  bool retVal = translator->load(QLocale(), fileName, QLatin1String("_"),
                                 QLatin1String(":/i18n"));
  if (retVal) {
  ...
  if (!retVal) {
    ...
    if ((lang.contains(QLatin1String("uk-UA"), Qt::CaseInsensitive)) ||
      ...
      QMessageBox::warning(nullptr, "Warning",
                           "Error loading localization resources.");
    }
  }
#else
  Q_ASSERT_X(false, Q_FUNC_INFO,
             "This code was tested and works correctly in Qt 6.3.2 only");
#endif
}

void createConfigInfo(const QString &appName, const QStringList &list,
                      QString &path, QString &fileName, bool &isRdsMode) {
#ifdef Q_OS_WIN
  ...
  if (errorCode != 0) {
    ...
    QMessageBox::warning(nullptr, "Warning",
                         QString("Error checking RDS mode: %1\nThe utility "
                                 "will be launched in the single user mode!")
                             .arg(msg));
  }
  if (isRdsMode ||
      list.contains(QLatin1String("--enablerds"), Qt::CaseSensitive)) {
    path =
        QStringLiteral("%AppData%/%1")
            .arg(appName); // %SystemDrive%:/Users/%UserName%/AppData/Roaming/%1
  }
#else
...
int main(int argc, char *argv[]) {
  ...
  if (sa.isRunning()) {
    QMessageBox::information(nullptr, QObject::tr("Information"),
                             QObject::tr("The another copy of the Utility is "
                                         "still running. Please close it."),
                             QMessageBox::Ok);
    return 1;
  }
  QString iniFile;
  ...
  if (!elcUtils::mkPath(appPath, errorString)) {
    QMessageBox::critical(nullptr, QObject::tr("Error"),
                          QObject::tr("Cannot create folder: %1\nDetails: %2")
                              .arg(appPath, errorString),
                          QMessageBox::Ok);
    return 1;
  }
  if (!CElcGuiAppSettings::instance().init(appPath, iniFile, isRdsMode)) {
    QMessageBox::critical(
        ...
        QObject::tr("The settings Class cannot be initialized."),
        QMessageBox::Ok);
    return 1;
  }
  ...
  if (dbName.isEmpty()) {
    ...
    QMessageBox::warning(
        ...
        QObject::tr("Unable to get database file name.\nThe database file will "
                    "be created on the default path."),
        QMessageBox::Ok);
  }
