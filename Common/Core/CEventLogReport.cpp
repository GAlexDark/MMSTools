...
CEventLogReport::CEventLogReport(QObject *parent) : CBasicReport{parent} {
  clearErrorString();
}

bool CEventLogReport::generateReport(const QString &arguments) {
  ...
  if (retVal) {
    ...
    while (m_db->isNext()) {
      ...
      setReportDataItem(&xlsxReport, "timestampISO8601", colTimestampISO8601,
                        row);
...
void CEventLogReport::setReportDataItem(QXlsx::Document *report,
