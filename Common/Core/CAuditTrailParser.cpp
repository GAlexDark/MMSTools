...
const QRegularExpression
    reAuditTrailHeader(QLatin1String("(^(.*?);(.*?);(.*?);(.*?);(.*?);(.*?))"));
const QRegularExpression
    rePersonData(QLatin1String("Person\\s\\[pk=\\d+,\\salias=(.*)\\]"));
const QRegularExpression
    reLoadAuditTrail(QLatin1String("^.*;.*;(.*);.*;.*;.*;.*;.*;.*$"));

const QString person(QLatin1String("Person ["));
const QString nullValue(QLatin1String("null"));

bool CAuditTrailParser::parsePersonDataDetails() {
  bool retVal = false;
  qsizetype firstPos = m_attributes.indexOf(person, 0);
  if (firstPos != -1) {
    qsizetype lastPos = m_attributes.indexOf(QLatin1Char(']'), firstPos);
    QString alias =
        m_attributes.sliced(firstPos, lastPos - firstPos + 1).trimmed();
...
bool CAuditTrailParser::parseLoadAuditTrail() {
  bool retVal = false;
...
bool CAuditTrailParser::parseAttributesDetails() {
  return parsePersonDataDetails();
}

CAuditTrailParser::CAuditTrailParser(QObject *parent) : CBasicParser{parent} {
  clearErrorString();
  initFfs(m_eolChars, m_quoteChar, m_delimiterChar);
}

bool CAuditTrailParser::parse(const QString &line) {
  bool retVal = false;
  ...
  if (match.hasMatch()) {
    ...
    qsizetype posEnd =
        m_username.indexOf(QLatin1Char(')'), posStart + 1) - posStart - 1;
...
void CAuditTrailParser::convertData(mms::dataItem_t &data) {
...
bool CAuditTrailParser::checkHeader(const QString &line) {
  QString columns;
...
QString CAuditTrailParser::insertString() const {
  return audittrail::insertData;
}

void CAuditTrailParser::getParsedData(QString &status, QDateTime &timestamp,
                                      QString &method, QString &username,
                                      QString &role, QString &companyname,
                                      QString &attributes, QString &username1,
                                      QString &internalip,
...
QString CAuditTrailParser::createTable() const {
