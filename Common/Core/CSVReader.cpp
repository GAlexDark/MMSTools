...
bool CTextFileReader::checkBOM() {
  ...
  if (bom != utf8bom) {
    ...
    m_errorString = QStringLiteral(
        "Wrong BOM header. The file must have UTF-8 BOM 0xef 0xbb 0xbf header");
  }

  return retVal;
}

qint64 CTextFileReader::indexOfEol(const qint64 startPos,
...
bool CTextFileReader::readColumnNames(const qint64 bytesRead, bool &isEOF,
                                      qint64 &prevPosition) {
  bool retVal = true;
...
bool CTextFileReader::readLargeFile() {
...
bool CTextFileReader::init(bool dataHasHeaders, const mms::ffs_t &ffs) {
  ...
  Q_ASSERT_X((m_eolCharsCount == 1) || (m_eolCharsCount == 2), Q_FUNC_INFO,
             "Wrong EOL chars");
...
bool CMmsLogsReader::initDB(const QString &dbFileName,
  ...
  if (retVal) {
    ...
    if (retVal) {
      ...
      dbCommandItems.append(pragmaSynchronous.arg(
          pragmaList->value(QLatin1String("synchronous"))));

      int blockSize = elcUtils::getStorageBlockSize(dbFileName);
      dbCommandItems.append(pragmaPageSize.arg(blockSize));

      dbCommandItems.append(pragmaJournalMode.arg(
          pragmaList->value(QLatin1String("journal_mode"))));
      dbCommandItems.append(
          pragmaTempStore.arg(pragmaList->value(QLatin1String("temp_store"))));
      dbCommandItems.append(pragmaLockingMode.arg(
          pragmaList->value(QLatin1String("locking_mode"))));
  ...
  if (retVal) {
    dataList_t res =
        m_db.findInDB(QLatin1String("PRAGMA journal_mode;"), false);
    QString value = "journal_mode: " +
                    QString::fromStdWString(res.at(0).at(0).toStdWString());
    __DEBUG(value)
    res.clear();
    res = m_db.findInDB(QLatin1String("PRAGMA page_size;"), false);
    value =
        "page_size: " + QString::fromStdWString(res.at(0).at(0).toStdWString());
    __DEBUG(value)
    res.clear();
    ...
    value =
        "mmap_size: " + QString::fromStdWString(res.at(0).at(0).toStdWString());
    __DEBUG(value)
    res.clear();
    res = m_db.findInDB(QLatin1String("pragma threads;"), false);
    value =
        "threads: " + QString::fromStdWString(res.at(0).at(0).toStdWString());
    __DEBUG(value)
    res.clear();
  }
#endif
...
CMmsLogsReader::~CMmsLogsReader() { m_db.close(); }

bool CMmsLogsReader::init(const quint16 logId, const QString &dbFileName,
                          bool dataHasHeaders,
  ...
  if (retVal) {
    ...
    retVal = CTextFileReader::init(dataHasHeaders,
                                   m_parser->fileFieldsSeparationInfo());
    if (retVal) {
...
bool CMmsLogsReader::checkHeader(const QString &line) {
  Q_CHECK_PTR(m_parser);
...
bool CMmsLogsReader::convertData(const QString &line) {
  ...
  if (retVal) {
    ...
    setErrorString(QStringLiteral("Parsing error at line number %1: %2")
                       .arg(getLineNumber())
                       .arg(line));
  }
  return retVal;
...
CMmsLogsThreadReader::CMmsLogsThreadReader(QObject *parent) : QThread{parent} {
  m_fileNames.clear();
}

void CMmsLogsThreadReader::run() {
  clearErrorString();
#ifdef QT_DEBUG
  QElapsedTimer timer;
  timer.start();
  ...
  if (m_fileNames.size() > 0) {
    ...
    while ((i < filesCount) && m_retVal) {
      ...
      if (!m_retVal) {
        ...
        emit sendMessage(
            tr("Reading of the file %1 has started.").arg(fileName));
      ...
      QString msg = m_retVal ? tr("The file %1 was read").arg(fileName)
                             : tr("The file %1 was not read").arg(fileName);
