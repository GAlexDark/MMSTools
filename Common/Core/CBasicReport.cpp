...
CBasicReport::CBasicReport(QObject *parent) : QObject{parent} {
  m_errorString.clear();
  m_reportFileName.clear();
}

void CBasicReport::init(pBasicDatabase db, const QString &reportName,
...
void CBasicReport::setDateTimeFormat(QXlsx::Format &dateFormat) {
  dateFormat.setHorizontalAlignment(QXlsx::Format::AlignRight);
  QString dateTimeFormat = m_showMilliseconds
                               ? QLatin1String("dd.mm.yyyy hh:mm:ss.000")
                               : QLatin1String("dd.mm.yyyy hh:mm:ss");
  dateFormat.setNumberFormat(dateTimeFormat);
}

QString CBasicReport::checkDetails(const QString &data) {
  QString buf = data;
...
void CBasicReport::setReportDataItem(QXlsx::Document *report,
                                     const pBasicDatabase db,
...
quint16 CBasicReport::reportID() const {
  QString value;
  return elcUtils::getMetaClassInfo(this, QLatin1String("ID"), value)
             ? value.toUInt()
             : 0;
}
