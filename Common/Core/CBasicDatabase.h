...
public:
  ...
  /*!
   * bool CBasicDatabase::init(const QString &dbDriverName, const QString
   * &connectionString). Inited the DB connection with the DB driver name \a
   * dbDriverName and the \a connectionString. If used QSLite, the \a
   * connectionString is the path to the DB file.
   */
...
private:
  ...
  bool m_isInited =
      false; // флаг подключения к БД. True - connected, false - not connected
