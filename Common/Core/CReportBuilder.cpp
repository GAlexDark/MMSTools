...
bool CReportBuilder::init(quint16 logID, const QString &dbFileName,
                          const QString &reportName,
                          const QStringList *excludedUsernamesList,
                          const QStringList *includedUsernamesList,
...
bool CReportBuilder::generateReport() {
  QString args;
  args.clear();
  ...
  if (isIncluded && isExcluded) {
    m_errorString = QStringLiteral("User exclusion and inclusion lists cannot "
    ...
    if (isIncluded || isExcluded) {
      ...
      if (isIncluded) {
        ...
        for (qsizetype i = 0; i < size; ++i) {
          args.append(QStringLiteral("username='%1' OR ")
                          .arg(m_includedUsernamesList.at(i)));
        } // for
        args.append(QStringLiteral("username='%1'")
                        .arg(m_includedUsernamesList.at(size)));
      }
      if (isExcluded) {
        qsizetype size = m_excludedUsernamesList.size() - 1;
        for (qsizetype i = 0; i < size; ++i) {
          args.append(QStringLiteral("username<>'%1' AND ")
                          .arg(m_excludedUsernamesList.at(i)));
        } // for
        args.append(QStringLiteral("username<>'%1'")
                        .arg(m_excludedUsernamesList.at(size)));
      }
...
bool CSVThreadReportBuilder::init(quint16 logID, const QString &dbFileName,
                                  const QString &reportName,
                                  const QStringList *excludedUsernamesList,
                                  const QStringList *includedUsernamesList,
                                  const bool showMilliseconds) {
  Q_CHECK_PTR(excludedUsernamesList);
  Q_CHECK_PTR(includedUsernamesList);
  return m_builder.init(logID, dbFileName, reportName, excludedUsernamesList,
                        includedUsernamesList, showMilliseconds);
}

void CSVThreadReportBuilder::run() {
  m_errorString.clear();
