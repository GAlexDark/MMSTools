...
bool CBasicDatabase::_exec(const QString &query) {
  m_SQLRes->clear();
...
bool CBasicDatabase::trunvateDB(const QString &connectionString,
                                QString &errorString, qsizetype tablesCount,
                                const QStringList &tablesNames,
...
bool CBasicDatabase::init(const QString &dbDriverName,
  ...
  if (!m_isInited) {
    if (connectionString.isEmpty() || dbDriverName.isEmpty()) {
      ...
      if (m_isInited) {
        ...
        m_errorString =
            QStringLiteral("Error loading DB driver: %1").arg(error.text());
      }
    }
  }
  return m_isInited;
...
bool CBasicDatabase::open() {
  ...
  if (!m_db.isOpen()) {
    ...
    if (retVal) {
      ...
      QSqlError error = m_db.lastError();
      m_errorString =
          QStringLiteral("Error open DB file: %1").arg(error.text());
    }
  }
  return retVal;
}

void CBasicDatabase::close() {
...
void CBasicDatabase::deinit() {
  close();
  _Deinit();
}

bool CBasicDatabase::beginTransaction() {
...
bool CBasicDatabase::commitTransaction() {
  bool retVal = true;
  if (m_isBeginTransaction) {
    if (m_SQLRes && m_SQLRes->isActive()) {
      m_SQLRes->finish();
    ...
    if (retVal) {
      ...
      QSqlError error = m_db.lastError();
      m_errorString = QStringLiteral("Transaction Error. Commit status: %1")
                          .arg(error.text());
    }
  }
  return retVal;
...
bool CBasicDatabase::rollbackTransaction() {
  bool retVal = true;
  if (m_isBeginTransaction) {
    retVal = m_db.rollback();
    if (!retVal) {
      QSqlError error = m_db.lastError();
      m_errorString = QStringLiteral("Transaction Error. Rollback status: %1")
                          .arg(error.text());
    }
    m_isBeginTransaction = false;
  }
...
bool CBasicDatabase::optimizeDatabaseSize() {
...
bool CBasicDatabase::truncateTable(const QString &tableName) {
  if (m_SQLRes->isActive()) {
    m_SQLRes->finish();
  }
  bool retVal =
      _exec(QLatin1String("drop table if exists [%1];").arg(tableName));
  if (retVal) {
...
bool CBasicDatabase::prepareRequest(const QString &query) {
  m_SQLRes->clear();
...
bool CBasicDatabase::execRequest(pDataItem data) {
  Q_CHECK_PTR(data);
...
bool CBasicDatabase::insertToDB(const QString &query, pDataItem data) {
  Q_CHECK_PTR(data);
...
dataList_t CBasicDatabase::findInDB(const QString &query,
                                    bool addColumnHeaders) {
...
bool CBasicDatabase::exec(const QString &query) {
