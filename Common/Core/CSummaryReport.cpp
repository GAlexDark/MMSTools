...
CSummaryReport::CSummaryReport(QObject *parent) : CBasicReport{parent} {
  clearErrorString();
}

bool CSummaryReport::generateReport(const QString &arguments) {
  ...
  if (retVal) {
    ...
    while (m_db->isNext()) {
      ...
      setReportDataItem(&xlsxReport, "details_or_attributes",
                        colDetailsOrAttributes, row);
...
void CSummaryReport::setReportDataItem(QXlsx::Document *report,
