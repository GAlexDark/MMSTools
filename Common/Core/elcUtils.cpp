...
bool elcUtils::sanitizeValue(const QString &pattern, const QString &value) {
  QRegularExpression mask(pattern);
  QRegularExpressionMatch match = mask.match(value);
  return match.hasMatch() ? true : false;
}

QString elcUtils::sanitizeValue(const QString &value,
  ...
  if (value.isEmpty()) {
    ...
    retVal = allowedValues.contains(value, Qt::CaseInsensitive) ? value
                                                                : defaultValue;
  }
  return retVal;
}

QStringList elcUtils::parseValuesList(QString data) {
  QStringList retVal;
  data = data.trimmed();
...
void elcUtils::parseValuesList(QStringList &data) {
  if (!data.isEmpty()) {
...
int elcUtils::getStorageBlockSize(const QString &file) {
...
QString elcUtils::getFormattedDateTime(const QString &dateTime) {
  QDateTime buildDate = QDateTime::fromString(dateTime);
  return buildDate.toString(QLatin1String("yyyyMMddhhmm"));
}

void elcUtils::waitForEndThread(QThread *obj, unsigned long time) {
  while (!obj->wait(time)) {
...
QString elcUtils::getWindowsApiErrorMessage(quint32 errorCode) {
  wchar_t *buf = nullptr;
  QString retVal(
      QStringLiteral("Unknown Windows API error: %1").arg(errorCode));
  const quint32 messageFlags = FORMAT_MESSAGE_ALLOCATE_BUFFER |
                               FORMAT_MESSAGE_FROM_SYSTEM |
                               FORMAT_MESSAGE_IGNORE_INSERTS;
  quint32 len = FormatMessage(messageFlags, NULL, errorCode,
...
bool elcUtils::isRdpMode(quint32 &errorCode) {
...
bool elcUtils::isRdsMode(quint32 &errorCode) {
...
void expandEnvStrings_windows(QString &path) {
  SetLastErrorEx(ERROR_SUCCESS, 0);
  DWORD bufSize =
      ExpandEnvironmentStrings(path.toStdWString().c_str(), NULL, 0);
  if (bufSize != 0) {
    QScopedPointer<wchar_t> fullPath(new wchar_t[bufSize]);
    Q_CHECK_PTR(fullPath);
    bufSize = ExpandEnvironmentStrings(path.toStdWString().c_str(),
                                       fullPath.data(), bufSize);
    if (bufSize != 0) {
...
void expandEnvStrings_linux(QString &path) {
...
void elcUtils::expandEnvironmentStrings(QString &path) {
#ifdef Q_OS_WIN
...
bool elcUtils::mkPath(const QString &dirPath, QString &errorString) {
...
QStringList elcUtils::getDataSourceList(const QString &path,
