...
const QRegularExpression
    reSuccessLogon(QLatin1String("^username:\\s(.*?),\\n\\s\\stype:\\s(.*?),"
                                 "\\n\\s\\sip\\saddress:\\s(.*?)$"));
const QRegularExpression reFailedLogon(
...
bool CEventLogParser::parseUserSuccessLogonDetails() {
...
bool CEventLogParser::parseUserFailedLogonDetails() {
...
bool CEventLogParser::parseUserLogonDetails() {
  ...
  if ((QString::compare(m_type, authSuccessUk, Qt::CaseInsensitive) == 0) ||
    ...
    if ((QString::compare(m_type, authFailedUk, Qt::CaseInsensitive) == 0) ||
      ...
      if (QString::compare(m_prevValueUFLD.details, m_details,
                           Qt::CaseInsensitive) == 0) {
        m_username1.clear();
...
CEventLogParser::CEventLogParser(QObject *parent) : CBasicParser{parent} {
  clearErrorString();
  initFfs(m_eolChars, m_quoteChar, m_delimiterChar);
}

bool CEventLogParser::parse(const QString &line) {
  m_details = line;
  ...
  if (match.hasMatch()) {
    ...
    if (m_timestamp.isValid()) {
      ...
      setErrorString(QStringLiteral("Error converting Timestamp value: %1")
                         .arg(m_timestampISO8601));
    }
...
void CEventLogParser::convertData(mms::dataItem_t &data) {
...
bool CEventLogParser::checkHeader(const QString &line) {
  QString columns;
...
QString CEventLogParser::insertString() const { return eventlog::insertData; }

void CEventLogParser::getParsedData(
    QString &username, QString &timestampISO8601, QString &requestID,
    QString &type, QString &details, QString &username1, QString &authType,
    QString &externalIP, QString &internalIP, QDateTime &timestampTZ) const {
  username = m_username;
...
QString CEventLogParser::createTable() const { return eventlog::createTable; }
