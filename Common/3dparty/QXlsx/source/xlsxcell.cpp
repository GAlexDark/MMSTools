...
CellPrivate::CellPrivate(const CellPrivate *const cp)
    : parent(cp->parent), cellType(cp->cellType), value(cp->value),
      formula(cp->formula), format(cp->format), richString(cp->richString),
      styleNumber(cp->styleNumber) {}

/*!
  \class Cell
  \inmodule QtXlsx
  \brief The Cell class provides a API that is used to handle the worksheet
  cell.
 ...
 */
// qint32 styleIndex = (-1)
Cell::Cell(const QVariant &data, CellType type, const Format &format,
           Worksheet *parent, qint32 styleIndex)
...
// https://www.qt.io/blog/whats-new-in-qmetatype-qvariant
    ...
    // double deciamlPointValue2 = deciamlPointValue1 - (double(hour) *
    // (double(1.0) / double(24.0))); double dMin = deciamlPointValue2 *
    // (double(1.0) / double(60.0)); int min = int(dMin);

    // double deciamlPointValue3 = deciamlPointValue2 - (double(min) *
    // (double(1.0) / double(60.0))); double dSec = deciamlPointValue3 *
    // (double(1.0) / double(60.0)); int sec = int(dSec);
...
bool Cell::isDateTime() const {
  ...
  // dev67
  if (cellType == NumberType || cellType == DateType ||
      cellType == CustomType) {
...
QDateTime Cell::dateTime() const
{
        ...
        return datetimeFromNumber(d->value.toDouble(),
d->parent->workbook()->isDate1904());
}
*/
