...
FormatPrivate::FormatPrivate()
    : dirty(true), font_dirty(true), font_index_valid(false), font_index(0),
      fill_dirty(true), fill_index_valid(false), fill_index(0),
      border_dirty(true), border_index_valid(false), border_index(0),
      xf_index(-1), xf_indexValid(false), is_dxf_fomat(false), dxf_index(-1),
      dxf_indexValid(false), theme(0) {}

FormatPrivate::FormatPrivate(const FormatPrivate &other)
    : QSharedData(other), dirty(other.dirty), formatKey(other.formatKey),
      font_dirty(other.font_dirty), font_index_valid(other.font_index_valid),
      font_key(other.font_key), font_index(other.font_index),
      fill_dirty(other.fill_dirty), fill_index_valid(other.fill_index_valid),
      fill_key(other.fill_key), fill_index(other.fill_index),
      border_dirty(other.border_dirty),
      border_index_valid(other.border_index_valid),
      border_key(other.border_key), border_index(other.border_index),
      xf_index(other.xf_index), xf_indexValid(other.xf_indexValid),
      is_dxf_fomat(other.is_dxf_fomat), dxf_index(other.dxf_index),
      dxf_indexValid(other.dxf_indexValid), theme(other.theme),
 ...
 * \class Format
 * \inmodule QtXlsx
 * \brief Providing the methods and properties that are available for formatting
 * cells in Excel.
 */
...
bool Format::isDateTimeFormat() const {
  ...
  if (hasProperty(FormatPrivate::P_NumFmt_FormatCode)) {
    ...
    if ((idx >= 27 && idx <= 36) ||
        (idx >= 50 && idx <= 58)) // Used in CHS\CHT\JPN\KOR
      return true;
  }
 ...
 */
int Format::fontSize() const { return intProperty(FormatPrivate::P_Font_Size); }
...
Format::FontScript Format::fontScript() const {
  return static_cast<Format::FontScript>(
      intProperty(FormatPrivate::P_Font_Script));
}
...
Format::FontUnderline Format::fontUnderline() const {
  return static_cast<Format::FontUnderline>(
      intProperty(FormatPrivate::P_Font_Underline));
}
...
QByteArray Format::fontKey() const {
  ...
  if (d->font_dirty) {
    ...
    for (int i = FormatPrivate::P_Font_STARTID; i < FormatPrivate::P_Font_ENDID;
         ++i) {
...
void Format::setHorizontalAlignment(HorizontalAlignment align) {
  ...
  if (hasProperty(FormatPrivate::P_Alignment_ShinkToFit) &&
      (align == AlignHFill || align == AlignHJustify ||
       align == AlignHDistributed)) {
...
/*!
 * Set the text rotation with the given \a rotation. Must be in the range [0,
 * 180] or 255.
 */
...
/*!
 * Set the text indentation level with the given \a indent. Must be less than or
 * equal to 15.
 */
void Format::setIndent(int indent) {
  if (indent && hasProperty(FormatPrivate::P_Alignment_AlignH)) {
    ...
    if (hl != AlignHGeneral && hl != AlignLeft && hl != AlignRight &&
        hl != AlignHJustify) {
...
bool Format::hasAlignmentData() const {
  ...
  for (int i = FormatPrivate::P_Alignment_STARTID;
       i < FormatPrivate::P_Alignment_ENDID; ++i) {
    if (hasProperty(i))
      return true;
  }
  return false;
...
Format::BorderStyle Format::leftBorderStyle() const {
  return static_cast<BorderStyle>(
      intProperty(FormatPrivate::P_Border_LeftStyle));
}
...
Format::BorderStyle Format::rightBorderStyle() const {
  return static_cast<BorderStyle>(
      intProperty(FormatPrivate::P_Border_RightStyle));
}
...
Format::BorderStyle Format::topBorderStyle() const {
  return static_cast<BorderStyle>(
      intProperty(FormatPrivate::P_Border_TopStyle));
}
...
Format::BorderStyle Format::bottomBorderStyle() const {
  return static_cast<BorderStyle>(
      intProperty(FormatPrivate::P_Border_BottomStyle));
}
...
Format::BorderStyle Format::diagonalBorderStyle() const {
  return static_cast<BorderStyle>(
      intProperty(FormatPrivate::P_Border_DiagonalStyle));
}
...
Format::DiagonalBorderType Format::diagonalBorderType() const {
  return static_cast<DiagonalBorderType>(
      intProperty(FormatPrivate::P_Border_DiagonalType));
}
...
Format::FillPattern Format::fillPattern() const {
  return static_cast<FillPattern>(
      intProperty(FormatPrivate::P_Fill_Pattern, PatternNone));
}
...
QByteArray Format::fillKey() const {
  ...
  if (d->fill_dirty) {
    QByteArray key;
    QDataStream stream(&key, QIODevice::WriteOnly);
    for (int i = FormatPrivate::P_Fill_STARTID; i < FormatPrivate::P_Fill_ENDID;
         ++i) {
      auto it = d->properties.constFind(i);
      if (it != d->properties.constEnd())
        stream << i << it.value();
...
/*!
        Merges the current format with the properties described by format \a
   modifier.
 */
...
int Format::theme() const { return d->theme; }
...
void Format::setProperty(int propertyId, const QVariant &value,
                         const QVariant &clearValue, bool detach) {
  if (!d)
    d = new FormatPrivate;
  ...
  if (propertyId >= FormatPrivate::P_Font_STARTID &&
      propertyId < FormatPrivate::P_Font_ENDID) {
    d->font_dirty = true;
