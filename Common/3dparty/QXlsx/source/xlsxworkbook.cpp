 ...
 */
bool Workbook::defineName(const QString &name, const QString &formula,
  ...
  d->definedNamesList.append(
      XlsxDefineNameData(name, formulaString, comment, id));
  return true;
}

AbstractSheet *Workbook::addSheet(const QString &name,
 ...
 */
AbstractSheet *Workbook::addSheet(const QString &name, int sheetId,
                                  AbstractSheet::SheetType type) {
  Q_D(Workbook);
...
bool Workbook::copySheet(int index, const QString &newName) {
  ...
  if (!newName.isEmpty()) {
    ...
    do {
      ++copy_index;
      worksheetName = QStringLiteral("%1(%2)")
                          .arg(d->sheets[index]->sheetName())
                          .arg(copy_index);
  ...
  ++d->last_sheet_id;
  AbstractSheet *sheet =
      d->sheets[index]->copy(worksheetName, d->last_sheet_id);
  d->sheets.append(QSharedPointer<AbstractSheet>(sheet));
  d->sheetNames.append(sheet->sheetName());
 ...
 * \internal
 */
QList<QSharedPointer<AbstractSheet>>
...
void Workbook::saveToXmlFile(QIODevice *device) const {
  ...
  writer.writeAttribute(QStringLiteral("defaultThemeVersion"),
                        QStringLiteral("124226"));

  writer.writeStartElement(QStringLiteral("bookViews"));
  writer.writeEmptyElement(QStringLiteral("workbookView"));
  writer.writeAttribute(QStringLiteral("xWindow"),
                        QString::number(d->x_window));
  writer.writeAttribute(QStringLiteral("yWindow"),
                        QString::number(d->y_window));
  writer.writeAttribute(QStringLiteral("windowWidth"),
                        QString::number(d->window_width));
  writer.writeAttribute(QStringLiteral("windowHeight"),
                        QString::number(d->window_height));
  // Store the firstSheet when it isn't the default
  // For example, when "the first sheet 0 is hidden", the first sheet will be 1
  if (d->firstsheet > 0)
    writer.writeAttribute(QStringLiteral("firstSheet"),
                          QString::number(d->firstsheet + 1));
  // Store the activeTab when it isn't the first sheet
  if (d->activesheetIndex > 0)
    writer.writeAttribute(QStringLiteral("activeTab"),
                          QString::number(d->activesheetIndex));
  ...
  for (int i = 0; i < d->sheets.size(); ++i) {
    ...
    writer.writeAttribute(QStringLiteral("sheetId"),
                          QString::number(sheet->sheetId()));
      ...
      if (data.sheetId != -1) {
        ...
        for (int i = 0; i < d->sheets.size(); ++i) {
          if (d->sheets[i]->sheetId() == data.sheetId) {
            writer.writeAttribute(QStringLiteral("localSheetId"),
                                  QString::number(i));
            break;
          }
        }
  ...
  if (!sharedStrings()->isEmpty())
    d->relationships->addDocumentRelationship(
        QStringLiteral("/sharedStrings"), QStringLiteral("sharedStrings.xml"));
}
    ...
    if (token == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("sheet")) {
        ...
        XlsxRelationship relationship =
            d->relationships->getRelationshipById(rId);
        ...
        // QDir::cleanPath(splitPath(strFilePath).constFirst() +
          ...
          if (reader.tokenType() == QXmlStreamReader::StartElement) {
            if (reader.name() == QLatin1String("workbookView")) {
              ...
              if (attrs.hasAttribute(QLatin1String("windowWidth")))
                d->window_width =
                    attrs.value(QLatin1String("windowWidth")).toInt();
              ...
              if (attrs.hasAttribute(QLatin1String("firstSheet")))
                d->firstsheet =
                    attrs.value(QLatin1String("firstSheet")).toInt();
        ...
        QSharedPointer<SimpleOOXmlFile> link(
            new SimpleOOXmlFile(F_LoadFromExists));
