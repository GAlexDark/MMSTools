...
bool Worksheet::writeString(int row, int column, const RichString &value,
  ...
  auto cell = std::make_shared<Cell>(value.toPlainString(),
                                     Cell::SharedStringType, fmt, this);
...
/*!
        ...
        Write string \a value to the cell (\a row, \a column) with the \a
...
bool Worksheet::writeInlineString(int row, int column, const QString &value,
  ...
  d->cellTable[row][column] =
      std::make_shared<Cell>(value, Cell::InlineStringType, fmt, this);
  return true;
}
...
/*!
        Write numeric \a value to the cell (\a row, \a column) with the \a
   format. Returns true on success.
*/
bool Worksheet::writeNumeric(int row, int column, double value,
  ...
  d->workbook->styles()->addXfFormat(fmt);
  d->cellTable[row][column] =
      std::make_shared<Cell>(value, Cell::NumberType, fmt, this);
  return true;
}

/*!
        \overload
        Write \a formula to the cell \a row_column with the \a format and \a
...
/*!
        Write \a formula_ to the cell (\a row, \a column) with the \a format and
   \a result. Returns true on success.
*/
      ...
      for (int c = range.firstColumn(); c <= range.lastColumn(); ++c) {
        if (!(r == row && c == column)) {
          if (Cell *cell = cellAt(r, c)) {
            ...
            auto newCell =
                std::make_shared<Cell>(result, Cell::NumberType, fmt, this);
...
bool Worksheet::writeBlank(int row, int column, const Format &format) {
  ...
  d->cellTable[row][column] =
      std::make_shared<Cell>(QVariant{}, Cell::NumberType, fmt, this);
...
/*!
        Write a bool \a value to the cell (\a row, \a column) with the \a
   format. Returns true on success.
 */
bool Worksheet::writeBool(int row, int column, bool value,
  ...
  d->workbook->styles()->addXfFormat(fmt);
  d->cellTable[row][column] =
      std::make_shared<Cell>(value, Cell::BooleanType, fmt, this);
...
/*!
        Write a QDateTime \a dt to the cell (\a row, \a column) with the \a
   format. Returns true on success.
 */
...
// dev67
bool Worksheet::writeDate(const CellReference &row_column, const QDate &dt,
                          const Format &format) {
  if (!row_column.isValid())
    return false;
...
// dev67
bool Worksheet::writeDate(int row, int column, const QDate &dt,
                          const Format &format) {
  Q_D(Worksheet);
  if (d->checkDimensions(row, column))
    return false;
  ...
  double value = datetimeToNumber(QDateTime(dt, QTime(0, 0, 0)),
                                  d->workbook->isDate1904());
...
/*!
        \overload
        Write a QUrl \a url to the cell \a row_column with the given \a format
...
/*!
        Write a QUrl \a url to the cell (\a row, \a column) with the given \a
   format \a display and \a tip. Returns true on success.
 */
...
int Worksheet::insertImage(int row, int column, const QImage &image) {
  ...
  */
  ...
  anchor->ext =
      QSize(int(image.width() * scaleX), int(image.height() * scaleY));
...
bool Worksheet::getImage(int row, int column, QImage &img) {
  ...
  for (int i = 0; i < d->drawing->anchors.size(); i++) {
    if (d->drawing->anchors[i]->row() == row &&
        d->drawing->anchors[i]->col() == column) {
...
/*!
        Merge a \a range of cells. The first cell should contain the data and
   the others should be blank. All cells will be applied the same style if a
   valid \a format is given. Returns true on success.
...
void Worksheet::saveToXmlFile(QIODevice *device) const {
  ...
  writer.writeAttribute(QStringLiteral("zeroHeight"),
                        xsdBoolean(d->sheetFormatProps.zeroHeight));
  ...
  // NOTE: empty element is not problem. but, empty structure of element is not
  // parsed by Excel.
  ...
  if (!d->Prid.isEmpty()) {
    ...
    if (!d->PuseFirstPageNumber.isEmpty()) {
      writer.writeAttribute(QStringLiteral("useFirstPageNumber"),
                            d->PuseFirstPageNumber);
    }

    if (!d->PfirstPageNumber.isEmpty()) {
      writer.writeAttribute(QStringLiteral("firstPageNumber"),
                            d->PfirstPageNumber);
    }
  ...
  if (!(d->ModdHeader.isNull()) || !(d->MoodFooter.isNull())) {
    ...
    if (!d->MoodalignWithMargins.isEmpty()) {
      writer.writeAttribute(QStringLiteral("alignWithMargins"),
                            d->MoodalignWithMargins);
    }
...
void WorksheetPrivate::saveXmlCellData(QXmlStreamWriter &writer, int row,
  ...
  else if ((rIt = rowsInfo.constFind(row)) != rowsInfo.constEnd() &&
           !(*rIt)->format.isEmpty())
    writer.writeAttribute(QStringLiteral("s"),
                          QString::number((*rIt)->format.xfIndex()));
  ...
  } else if (cell->cellType() == Cell::NumberType) // 'n'
  {
    ...
    if (cell->value()
            .isValid()) { // note that, invalid value means 'v' is blank
...
double Worksheet::columnWidth(int column) {
  ...
  if (columnInfoList.size() == 0) {
    ...
    // qDebug() << "[debug]" << __FUNCTION__ <<  "column (info) is not found. "
    // << column;
  }

  if (columnInfoList.count() == 1) {
    // column information is found
    // qDebug() << "[debug]" << __FUNCTION__ <<  "column (info) is found. " <<
    // column << oneColWidth;
...
bool Worksheet::setRowHeight(int rowFirst, int rowLast, double height) {
  ...
  const QList<QSharedPointer<XlsxRowInfo>> rowInfoList =
...
double Worksheet::rowHeight(int row) {
  ...
  if (d->checkDimensions(row, min_col, false, true) ||
      it == d->rowsInfo.constEnd()) {
    return d->sheetFormatProps
        .defaultRowHeight; // return default on invalid row
  }
...
bool Worksheet::groupRows(int rowFirst, int rowLast, bool collapsed) {
  ...
  if (collapsed) {
    ...
    if (it == d->rowsInfo.end())
      it = d->rowsInfo.insert(rowLast + 1,
  ...
  for (int idx = 0; idx < nodes.size(); ++idx) {
    ...
    if (it != d->colsInfo.constEnd()) {
      ...
      QSharedPointer<XlsxColumnInfo> info(
          new XlsxColumnInfo(colStart, colEnd, false));
      info->outlineLevel += 1;
  ...
  while (!reader.atEnd() &&
    ...
    if (reader.readNextStartElement()) {
      ...
      } else if (reader.name() == QLatin1String("c")) // Cell
      {
        ...
        auto cell =
            std::make_shared<Cell>(QVariant{}, cellType, format, q, styleIndex);
          ...
          if (reader.readNextStartElement()) {
            ...
            } else if (reader.name() == QLatin1String("v")) // Value
            {
              ...
              if (cellType == Cell::SharedStringType) {
                ...
                QVariant vDatetimeValue =
                    datetimeFromNumber(dValue, bIsDate1904);
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("col")) {
        ...
        if (colAttrs.hasAttribute(QLatin1String("outlineLevel"))) {
          info->outlineLevel =
              colAttrs.value(QLatin1String("outlineLevel")).toInt();
        }

        // qDebug() << "[debug] " << __FUNCTION__ << min << max << info->width
        // << hasWidth;
  ...
  while (!reader.atEnd()) {
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("dimension")) {
        ...
        d->PpaperSize =
            attributes.value(QLatin1String("paperSize")).toString().trimmed();
        d->Pscale =
            attributes.value(QLatin1String("scale")).toString().trimmed();
        d->PfirstPageNumber = attributes.value(QLatin1String("firstPageNumber"))
                                  .toString()
                                  .trimmed();
        d->Porientation =
            attributes.value(QLatin1String("orientation")).toString().trimmed();
        d->PuseFirstPageNumber =
            attributes.value(QLatin1String("useFirstPageNumber"))
                .toString()
                .trimmed();
        d->PhorizontalDpi = attributes.value(QLatin1String("horizontalDpi"))
                                .toString()
                                .trimmed();
        d->PverticalDpi =
        ...
        d->Pcopies =
            attributes.value(QLatin1String("copies")).toString().trimmed();
      } else if (reader.name() == QLatin1String("pageMargins")) {
        QXmlStreamAttributes attributes = reader.attributes();

        d->PMfooter =
            attributes.value(QLatin1String("footer")).toString().trimmed();
        d->PMheader =
            attributes.value(QLatin1String("header")).toString().trimmed();
        d->PMbotton =
            attributes.value(QLatin1String("bottom")).toString().trimmed();
        d->PMtop = attributes.value(QLatin1String("top")).toString().trimmed();
        d->PMright =
            attributes.value(QLatin1String("right")).toString().trimmed();
        d->PMleft =
            attributes.value(QLatin1String("left")).toString().trimmed();
