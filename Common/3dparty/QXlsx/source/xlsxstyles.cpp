...
Styles::Styles(CreateFlag flag)
    : AbstractOOXmlFile(flag), m_nextCustomNumFmtId(176),
      m_isIndexedColorsDefault(true), m_emptyFormatAdded(false) {
...
void Styles::fixNumFmt(const Format &format) {
  ...
  if (m_builtinNumFmtsHash.isEmpty()) {
    ...
    //            m_builtinNumFmtsHash.insert(QStringLiteral("($#,##0_);($#,##0)"),
    //            5);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("($#,##0_);[Red]($#,##0)"),
    //            6);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("($#,##0.00_);($#,##0.00)"),
    //            7);
    ...
    m_builtinNumFmtsHash.insert(QStringLiteral("(#,##0.00_);[Red](#,##0.00)"),
                                40);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("_(* #,##0_);_(*
    //            (#,##0);_(*
    //            \"-\"_);_(_)"), 41);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("_($*
    //            #,##0_);_($* (#,##0);_($* \"-\"_);_(_)"), 42);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("_(* #,##0.00_);_(*
    //            (#,##0.00);_(*
    //            \"-\"??_);_(_)"), 43);
    //            m_builtinNumFmtsHash.insert(QStringLiteral("_($*
  ...
  if (!str.isEmpty()) {
    ...
    } else if ((cIt = m_customNumFmtsHash.constFind(str)) !=
               m_customNumFmtsHash.constEnd()) {
      const_cast<Format *>(&format)->fixNumberFormat(cIt.value()->formatIndex,
                                                     str);
    } else {
      // Assign a new fmt Id.
    ...
    if (it != m_customNumFmtIdMap.constEnd()) {
      const_cast<Format *>(&format)->fixNumberFormat(id,
                                                     it.value()->formatString);
    } else {
      bool found = false;
      for (auto &&it = m_builtinNumFmtsHash.constBegin();
           it != m_builtinNumFmtsHash.constEnd(); ++it) {
        if (it.value() == id) {
      ...
      if (!found) {
        // Wrong numFmt
        const_cast<Format *>(&format)->fixNumberFormat(
            id, QStringLiteral("General"));
      }
    }
  }
}
...
/*
   ...
   When \a force is true, add the format to the format list, even other format
   has the same key have been in. This is useful when reading existing .xlsx
   files which may contains duplicated formats.
*/
void Styles::addXfFormat(const Format &format, bool force) {
  ...
  if (fontIt == m_fontsHash.constEnd()) {
    // Still a valid font if the format has no fontData. (All font properties
    // are default)
  ...
  if (fillIt == m_fillsHash.constEnd()) {
    // Still a valid fill if the format has no fillData. (All fill properties
    // are default)
  ...
  if (borderIt == m_bordersHash.constEnd()) {
    // Still a valid border if the format has no borderData. (All border
...
void Styles::addDxfFormat(const Format &format, bool force) {
  ...
  if (!format.isEmpty() && !format.dxfIndexValid()) {
    if (formatIt ==
        m_dxf_formatsHash
            .constEnd()) // m_xf_formatsHash.constEnd()) // issue #108
    {
...
void Styles::saveToXmlFile(QIODevice *device) const {
  ...
  writer.writeAttribute(QStringLiteral("defaultTableStyle"),
                        QStringLiteral("TableStyleMedium9"));
  writer.writeAttribute(QStringLiteral("defaultPivotStyle"),
                        QStringLiteral("PivotStyleLight16"));
...
void Styles::writeNumFmts(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_customNumFmtIdMap.count()));

  QMapIterator<int, QSharedPointer<XlsxFormatNumberData>> it(
      m_customNumFmtIdMap);
  while (it.hasNext()) {
    it.next();
    writer.writeEmptyElement(QStringLiteral("numFmt"));
    writer.writeAttribute(QStringLiteral("numFmtId"),
                          QString::number(it.value()->formatIndex));
    writer.writeAttribute(QStringLiteral("formatCode"),
                          it.value()->formatString);
  }
...
void Styles::writeFonts(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_fontsList.count()));
...
void Styles::writeFont(QXmlStreamWriter &writer, const Format &format,
                       bool isDxf) const {
    ...
    if (u != Format::FontUnderlineNone) {
      ...
      if (u == Format::FontUnderlineDouble)
        ...
        writer.writeAttribute(QStringLiteral("val"),
                              QStringLiteral("singleAccounting"));
      else if (u == Format::FontUnderlineDoubleAccounting)
        writer.writeAttribute(QStringLiteral("val"),
                              QStringLiteral("doubleAccounting"));
    }
  }
  ...
  if (!isDxf && format.hasProperty(FormatPrivate::P_Font_Size)) {
    ...
    writer.writeAttribute(QStringLiteral("val"),
                          QString::number(format.fontSize()));
  }

  if (format.hasProperty(FormatPrivate::P_Font_Color)) {
    XlsxColor color =
        format.property(FormatPrivate::P_Font_Color).value<XlsxColor>();
...
void Styles::writeFills(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_fillsList.size()));
...
void Styles::writeFill(QXmlStreamWriter &writer, const Format &fill,
                       bool isDxf) const {
  ...
  // For normal fill formats, Excel prefer to outputting the default "none"
  // attribute But for dxf, Excel prefer to omitting the default "none" Though
  // not make any difference, but it make easier to compare origin files with
  // generate files during debug
  if (!(pattern == Format::PatternNone && isDxf))
    writer.writeAttribute(QStringLiteral("patternType"),
                          patternStrings[pattern]);
  // For a solid fill, Excel reverses the role of foreground and background
  // colours
...
void Styles::writeBorders(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_bordersList.count()));
...
void Styles::writeBorder(QXmlStreamWriter &writer, const Format &border,
                         bool isDxf) const {
  ...
  if (border.hasProperty(FormatPrivate::P_Border_DiagonalType)) {
    ...
    if (t == Format::DiagonalBorderUp) {
      ...
      writer.writeAttribute(QStringLiteral("diagonalDown"),
                            QStringLiteral("1"));
...
void Styles::writeSubBorder(QXmlStreamWriter &writer, const QString &type,
                            int style, const XlsxColor &color) const {
...
void Styles::writeCellXfs(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_xf_formatsList.size()));
  for (const Format &format : m_xf_formatsList) {
    int xf_id = 0;
    writer.writeStartElement(QStringLiteral("xf"));
    writer.writeAttribute(QStringLiteral("numFmtId"),
                          QString::number(format.numberFormatIndex()));
    writer.writeAttribute(QStringLiteral("fontId"),
                          QString::number(format.fontIndex()));
    writer.writeAttribute(QStringLiteral("fillId"),
                          QString::number(format.fillIndex()));
    writer.writeAttribute(QStringLiteral("borderId"),
                          QString::number(format.borderIndex()));
    writer.writeAttribute(QStringLiteral("xfId"), QString::number(xf_id));
    if (format.hasNumFmtData())
      writer.writeAttribute(QStringLiteral("applyNumberFormat"),
                            QStringLiteral("1"));
    ...
    if (format.hasAlignmentData())
      writer.writeAttribute(QStringLiteral("applyAlignment"),
                            QStringLiteral("1"));
    ...
    if (format.hasAlignmentData()) {
      ...
      if (format.hasProperty(FormatPrivate::P_Alignment_Indent))
        writer.writeAttribute(QStringLiteral("indent"),
                              QString::number(format.indent()));
      if (format.hasProperty(FormatPrivate::P_Alignment_Wrap) &&
          format.textWrap())
        writer.writeAttribute(QStringLiteral("wrapText"), QStringLiteral("1"));
      if (format.hasProperty(FormatPrivate::P_Alignment_ShinkToFit) &&
          format.shrinkToFit())
        writer.writeAttribute(QStringLiteral("shrinkToFit"),
                              QStringLiteral("1"));
...
void Styles::writeDxfs(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_dxf_formatsList.size()));
...
void Styles::writeColors(QXmlStreamWriter &writer) const {
  ...
  for (const QColor &color : m_indexedColors) {
    ...
    writer.writeAttribute(QStringLiteral("rgb"),
                          XlsxColor::toARGBString(color));
  }
...
bool Styles::readNumFmts(QXmlStreamReader &reader) {
  ...
  const auto count =
      hasCount ? attributes.value(QLatin1String("count")).toInt() : -1;
  ...
  while (!reader.atEnd() &&
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("numFmt")) {
        ...
        fmt->formatString =
            attributes.value(QLatin1String("formatCode")).toString();
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("patternFill")) {
        ...
        if (attributes.hasAttribute(QLatin1String("patternType"))) {
          ...
          fill.setFillPattern(it != patternValues.constEnd()
                                  ? it.value()
                                  : Format::PatternNone);
  ...
  while (!reader.atEnd() &&
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("left") ||
          reader.name() == QLatin1String("right") ||
          reader.name() == QLatin1String("top") ||
          reader.name() == QLatin1String("bottom") ||
  ...
  while (!reader.atEnd() &&
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("xf")) {
        ...
        //            qDebug()<<"... "<<i<<"
        //            "<<xfAttrs[i].name()<<xfAttrs[i].value();

        if (xfAttrs.hasAttribute(QLatin1String("numFmtId"))) {
          const auto numFmtIndex =
              xfAttrs.value(QLatin1String("numFmtId")).toInt();
        ...
        if (xfAttrs.hasAttribute(QLatin1String("fontId"))) {
          ...
          if (fontIndex >= m_fontsList.size()) {
            ...
            const auto apply = parseXsdBoolean(
                xfAttrs.value(QLatin1String("applyFont")).toString());
            if (apply) {
        ...
        if (xfAttrs.hasAttribute(QLatin1String("borderId"))) {
          ...
          if (id >= m_bordersList.size()) {
            ...
            const auto apply = parseXsdBoolean(
                xfAttrs.value(QLatin1String("applyBorder")).toString());
            if (apply) {
        ...
        const auto apply = parseXsdBoolean(
            xfAttrs.value(QLatin1String("applyAlignment")).toString());
        if (apply) {
          reader.readNextStartElement();
          if (reader.name() == QLatin1String("alignment")) {
            ...
            if (alignAttrs.hasAttribute(QLatin1String("horizontal"))) {
              static const QMap<QString, Format::HorizontalAlignment>
                  alignStringMap = {
            ...
            if (alignAttrs.hasAttribute(QLatin1String("vertical"))) {
              static const QMap<QString, Format::VerticalAlignment>
                  alignStringMap = {
            ...
            if (alignAttrs.hasAttribute(QLatin1String("indent"))) {
              const auto indent =
                  alignAttrs.value(QLatin1String("indent")).toInt();
  ...
  while (!reader.atEnd() &&
    ...
    if (reader.tokenType() == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("rgbColor")) {
        const auto &color =
            reader.attributes().value(QLatin1String("rgb")).toString();
...
QColor Styles::getColorByIndex(int idx) {
  if (m_indexedColors.isEmpty()) {
    m_indexedColors = {
        QColor(QRgba64::fromArgb32(0xFF000000)),
        QColor(QRgba64::fromArgb32(0xFFFFFFFF)),
        QColor(QRgba64::fromArgb32(0xFFFF0000)),
        QColor(QRgba64::fromArgb32(0xFF00FF00)),
        QColor(QRgba64::fromArgb32(0xFF0000FF)),
        QColor(QRgba64::fromArgb32(0xFFFFFF00)),
        QColor(QRgba64::fromArgb32(0xFFFF00FF)),
        QColor(QRgba64::fromArgb32(0xFF00FFFF)),
        QColor(QRgba64::fromArgb32(0xFF000000)),
        QColor(QRgba64::fromArgb32(0xFFFFFFFF)),
        QColor(QRgba64::fromArgb32(0xFFFF0000)),
        QColor(QRgba64::fromArgb32(0xFF00FF00)),
        ...
        QColor(QRgba64::fromArgb32(0xFFFF00FF)),
        QColor(QRgba64::fromArgb32(0xFF00FFFF)),
        QColor(QRgba64::fromArgb32(0xFF800000)),
        QColor(QRgba64::fromArgb32(0xFF008000)),
        QColor(QRgba64::fromArgb32(0xFF000080)),
        QColor(QRgba64::fromArgb32(0xFF808000)),
        QColor(QRgba64::fromArgb32(0xFF800080)),
        QColor(QRgba64::fromArgb32(0xFF008080)),
        QColor(QRgba64::fromArgb32(0xFFC0C0C0)),
        QColor(QRgba64::fromArgb32(0xFF808080)),
        QColor(QRgba64::fromArgb32(0xFF9999FF)),
        QColor(QRgba64::fromArgb32(0xFF993366)),
        QColor(QRgba64::fromArgb32(0xFFFFFFCC)),
        QColor(QRgba64::fromArgb32(0xFFCCFFFF)),
        QColor(QRgba64::fromArgb32(0xFF660066)),
        QColor(QRgba64::fromArgb32(0xFFFF8080)),
        QColor(QRgba64::fromArgb32(0xFF0066CC)),
        QColor(QRgba64::fromArgb32(0xFFCCCCFF)),
        QColor(QRgba64::fromArgb32(0xFF000080)),
        QColor(QRgba64::fromArgb32(0xFFFF00FF)),
        QColor(QRgba64::fromArgb32(0xFFFFFF00)),
        QColor(QRgba64::fromArgb32(0xFF00FFFF)),
        ...
        QColor(QRgba64::fromArgb32(0xFF008080)),
        QColor(QRgba64::fromArgb32(0xFF0000FF)),
        QColor(QRgba64::fromArgb32(0xFF00CCFF)),
        QColor(QRgba64::fromArgb32(0xFFCCFFFF)),
        QColor(QRgba64::fromArgb32(0xFFCCFFCC)),
        QColor(QRgba64::fromArgb32(0xFFFFFF99)),
        QColor(QRgba64::fromArgb32(0xFF99CCFF)),
        QColor(QRgba64::fromArgb32(0xFFFF99CC)),
        QColor(QRgba64::fromArgb32(0xFFCC99FF)),
        QColor(QRgba64::fromArgb32(0xFFFFCC99)),
        QColor(QRgba64::fromArgb32(0xFF3366FF)),
        QColor(QRgba64::fromArgb32(0xFF33CCCC)),
        QColor(QRgba64::fromArgb32(0xFF99CC00)),
        QColor(QRgba64::fromArgb32(0xFFFFCC00)),
        QColor(QRgba64::fromArgb32(0xFFFF9900)),
        QColor(QRgba64::fromArgb32(0xFFFF6600)),
        QColor(QRgba64::fromArgb32(0xFF666699)),
        QColor(QRgba64::fromArgb32(0xFF969696)),
        QColor(QRgba64::fromArgb32(0xFF003366)),
        QColor(QRgba64::fromArgb32(0xFF339966)),
        QColor(QRgba64::fromArgb32(0xFF003300)),
        QColor(QRgba64::fromArgb32(0xFF333300)),
        QColor(QRgba64::fromArgb32(0xFF993300)),
        QColor(QRgba64::fromArgb32(0xFF993366)),
        QColor(QRgba64::fromArgb32(0xFF333399)),
        QColor(QRgba64::fromArgb32(0xFF333333)),
    };
