...
int SharedStrings::count() const { return m_stringCount; }

bool SharedStrings::isEmpty() const { return m_stringList.isEmpty(); }
...
void SharedStrings::writeRichStringPart_rPr(QXmlStreamWriter &writer,
...
void SharedStrings::saveToXmlFile(QIODevice *device) const {
  ...
  writer.writeAttribute(QStringLiteral("count"),
                        QString::number(m_stringCount));
  writer.writeAttribute(QStringLiteral("uniqueCount"),
                        QString::number(m_stringList.size()));
...
void SharedStrings::readRichStringPart(QXmlStreamReader &reader,
                                       RichString &richString) {
...
void SharedStrings::readPlainStringPart(QXmlStreamReader &reader,
                                        RichString &richString) {
...
bool SharedStrings::loadFromXmlFile(QIODevice *device) {
    ...
    if (token == QXmlStreamReader::StartElement) {
      if (reader.name() == QLatin1String("sst")) {
        ...
        if ((hasUniqueCountAttr =
                 attributes.hasAttribute(QLatin1String("uniqueCount"))))
  ...
  if (m_stringList.size() != m_stringTable.size()) {
    ...
    // Nothing we can do here, as indices of the strings will be used when
    // loading sheets.
  }
