...
std::string copyTag(const std::string &sFrom, const std::string &sTo,
  ...
  while (true) {
    ...
    if (startPos != std::string::npos) {
      ...
      if (endPos == std::string::npos) { // second try to find the ending, maybe
                                         // it is "/>"
  ...
  if (!sFromData.empty()) { // tag found in 'from'?
                            // search all occurrences of tag in 'sOut' and
                            // delete them
    int firstPosTag = -1;
    while (true) {
      ...
      if (startPos != std::string::npos) {
        ...
        std::string tagEndTmp = tagEnd;
        if (endPos == std::string::npos) { // second try to find the ending,
                                           // maybe it is "/>"
    ...
    if (firstPosTag == -1) {
      ...
      std::vector<std::string> defaultPos{"</styleSheet>", "<pageMargins",
                                          "</workbook>"};
    ...
    if (firstPosTag >= 0) {
      ...
      std::string stringAfter =
          sOut.substr(firstPosTag, sOut.length() - firstPosTag);
...
void DocumentPrivate::init() {
  if (!contentTypes)
    contentTypes =
        std::make_shared<ContentTypes>(ContentTypes::F_NewFromScratch);

  if (workbook.isNull())
    workbook =
        QSharedPointer<Workbook>(new Workbook(Workbook::F_NewFromScratch));
}
...
bool DocumentPrivate::loadPackage(QIODevice *device) {
  ...
  contentTypes->loadFromXmlData(
      zipReader.fileData(QStringLiteral("[Content_Types].xml")));
  ...
  // load core property
  QList<XlsxRelationship> rels_core = rootRels.packageRelationships(
      QStringLiteral("/metadata/core-properties"));
  ...
  QList<XlsxRelationship> rels_styles =
      workbook->relationships()->documentRelationships(
  ...
  if (!rels_sharedStrings.isEmpty()) {
    ...
    workbook->d_func()->sharedStrings->loadFromXmlData(
        zipReader.fileData(path));
  }
...
bool DocumentPrivate::savePackage(QIODevice *device) const {
  ...
  for (int i = 0; i < worksheets.size(); ++i) {
    ...
    if (!rel->isEmpty())
      zipWriter.addFile(
          QStringLiteral("xl/worksheets/_rels/sheet%1.xml.rels").arg(i + 1),
          rel->saveToXmlData());
  }
  ...
  if (!chartsheets.isEmpty())
    docPropsApp.addHeadingPair(QStringLiteral("Chartsheets"),
                               chartsheets.size());
  for (int i = 0; i < chartsheets.size(); ++i) {
    ...
    if (!rel->isEmpty())
      zipWriter.addFile(
          QStringLiteral("xl/chartsheets/_rels/sheet%1.xml.rels").arg(i + 1),
          rel->saveToXmlData());
  }
  ...
  for (int i = 0; i < workbook->d_func()->externalLinks.count(); ++i) {
    SimpleOOXmlFile *link = workbook->d_func()->externalLinks[i].data();
    contentTypes->addExternalLinkName(
        QStringLiteral("externalLink%1").arg(i + 1));

    zipWriter.addFile(
        QStringLiteral("xl/externalLinks/externalLink%1.xml").arg(i + 1),
        link->saveToXmlData());
    Relationships *rel = link->relationships();
    if (!rel->isEmpty())
      zipWriter.addFile(
          QStringLiteral("xl/externalLinks/_rels/externalLink%1.xml.rels")
              .arg(i + 1),
          rel->saveToXmlData());
  }
  ...
  zipWriter.addFile(QStringLiteral("xl/workbook.xml"),
                    workbook->saveToXmlData());
  ...
  for (int i = 0; i < workbook->drawings().size(); ++i) {
    ...
    if (!drawing->relationships()->isEmpty())
      zipWriter.addFile(
          QStringLiteral("xl/drawings/_rels/drawing%1.xml.rels").arg(i + 1),
  ...
  zipWriter.addFile(QStringLiteral("docProps/app.xml"),
                    docPropsApp.saveToXmlData());
  zipWriter.addFile(QStringLiteral("docProps/core.xml"),
                    docPropsCore.saveToXmlData());
  ...
  zipWriter.addFile(QStringLiteral("xl/calcChain.xml"),
                    workbook->styles()->saveToXmlData());

  // save styles xml file
  contentTypes->addStyles();
  zipWriter.addFile(QStringLiteral("xl/styles.xml"),
                    workbook->styles()->saveToXmlData());

  // save theme xml file
  contentTypes->addTheme();
  zipWriter.addFile(QStringLiteral("xl/theme/theme1.xml"),
                    workbook->theme()->saveToXmlData());
  ...
  for (int i = 0; i < workbook->chartFiles().size(); ++i) {
    ...
    zipWriter.addFile(QStringLiteral("xl/charts/chart%1.xml").arg(i + 1),
                      cf->saveToXmlData());
  }
  ...
  for (int i = 0; i < mfs.size(); ++i) {
    ...
    zipWriter.addFile(
        QStringLiteral("xl/media/image%1.%2").arg(i + 1).arg(mf->suffix()),
        mf->contents());
  }
  ...
  zipWriter.addFile(QStringLiteral("[Content_Types].xml"),
                    contentTypes->saveToXmlData());
...
bool DocumentPrivate::copyStyle(const QString &from, const QString &to) {
  // create a temp file because the zip writer cannot modify already existing
  // zips
  ...
  QSharedPointer<ZipReader> toReader =
      QSharedPointer<ZipReader>(new ZipReader(to));
  ...
  for (int i = 0; i < toFilePaths.size(); i++) {
    if (toFilePaths[i].contains(QLatin1String("xl/styles"))) {
      if (filePaths.contains(toFilePaths[i])) { // style file exist in 'from' as
                                                // well modify style file
        std::string fromData =
        ...
        temporalZip.addFile(toFilePaths.at(i),
                            QString::fromUtf8(toData.c_str()).toUtf8());
    ...
    if (toFilePaths[i].contains(QLatin1String("xl/workbook"))) {
      if (filePaths.contains(toFilePaths[i])) { // workbook file exist in 'from'
                                                // as well modify workbook file
        std::string fromData =
            QString::fromUtf8(zipReader.fileData(toFilePaths[i])).toStdString();
        std::string toData =
            QString::fromUtf8(toReader->fileData(toFilePaths[i])).toStdString();
    ...
    if (toFilePaths[i].contains(QLatin1String("xl/worksheets/sheet"))) {
      if (filePaths.contains(toFilePaths[i])) { // sheet file exist in 'from' as
                                                // well modify sheet file
        std::string fromData =
            QString::fromUtf8(zipReader.fileData(toFilePaths[i])).toStdString();
        std::string toData =
            QString::fromUtf8(toReader->fileData(toFilePaths[i])).toStdString();
        ...
        toData =
            xlsxDocumentCpp::copyTag(fromData, toData, "conditionalFormatting");
        temporalZip.addFile(toFilePaths.at(i),
                            QString::fromUtf8(toData.c_str()).toUtf8());
        continue;
      }
...
/*!
  \class Document
  \inmodule QtXlsx
  \brief The Document class provides a API that is used to handle the contents
  of .xlsx files.
 ...
 */
bool Document::write(const CellReference &row_column, const QVariant &value,
                     const Format &format) {
 ...
 */
bool Document::write(int row, int col, const QVariant &value,
                     const Format &format) {
  if (Worksheet *sheet = currentWorksheet())
...
/*!
 * Insert an \a image to current active worksheet at the position \a row, \a
 * column Returns true if success.
 */
...
/*!
  ...
  others should be blank. All cells will be applied the same style if a valid \a
  format is given. Returns true on success.
...
/*!
  Sets width in characters for columns [\a colFirst, \a colLast]. Columns are
...
/*!
  ...
  Returns true on success.
 */
bool Document::setColumnFormat(int colFirst, int colLast,
                               const Format &format) {
  if (Worksheet *sheet = currentWorksheet())
...
/*!
  Sets the \a hidden property of the rows including and between \a rowFirst and
...
/*!
 *  Add a data \a validation rule for current worksheet. Returns true if
 * successful.
 */
...
/*!
 *  Add a  conditional formatting \a cf for current worksheet. Returns true if
 * successful.
 */
...
/*!
 * \brief Create a defined name in the workbook with the given \a name, \a
 * formula, \a comment and \a scope.
 *
 ...
 */
bool Document::defineName(const QString &name, const QString &formula,
                          const QString &comment, const QString &scope) {
  Q_D(Document);
...
/*!
        ...
        \endlist
*/
void Document::setDocumentProperty(const QString &key,
...
/*!
 * Returns the names of all properties that were addedusing
 * setDocumentProperty().
 */
...
/*!
 * Creates and inserts an document with the given \a name and \a type at the \a
 * index. Returns false if the \a name already used.
 */
bool Document::insertSheet(int index, const QString &name,
 ...
 * If the type of sheet is not AbstractSheet::ST_WorkSheet, then 0 will be
 * returned.
 */
...
bool Document::save() const {
  Q_D(const Document);
  QString name =
      d->packageName.isEmpty() ? d->defaultPackageName : d->packageName;
...
bool Document::load() const { return isLoadPackage(); }
 ...
 */
Document::~Document() { delete d_ptr; }
...
QMap<int, int> Document::getMaximalColumnWidth(int firstRow, int lastRow) {
  ...
  if (!currentWorksheet())
    return colWidth;
  QVector<CellLocation> cellLocation =
      currentWorksheet()->getFullCells(&maxRows, &maxCols);
...
bool Document::autosizeColumnWidth(const CellRange &range) {
  ...
  const QMap<int, int> colWidth =
      getMaximalColumnWidth(range.firstRow(), range.lastRow());
...
/*!
  Auto sets width in characters for columns [\a colFirst, \a colLast]. Columns
  are 1-indexed. Returns true on success.
 */
