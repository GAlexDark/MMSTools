 ...
 *
 * \value None the type of data is unrestricted. This is the same as not
 * applying a data validation. \value Whole restricts the cell to integer
 * values. Means "Whole number"? \value Decimal restricts the cell to decimal
 * values. \value List restricts the cell to a set of user specified values.
 ...
 * \value Custom restricts the cell based on an external Excel formula that
 ...
 */
DataValidation::DataValidation(ValidationType type, ValidationOperator op,
...
/*!
    Assigns \a other to this validation and returns a reference to this
   validation.
 */
 ...
 */
QString DataValidation::formula1() const { return d->formula1; }

/*!
    Returns the formula2.
 */
QString DataValidation::formula2() const { return d->formula2; }

/*!
    Returns whether blank is allowed.
 */
bool DataValidation::allowBlank() const { return d->allowBlank; }

/*!
    Returns the error message.
 */
QString DataValidation::errorMessage() const { return d->errorMessage; }
 ...
 */
QString DataValidation::promptMessage() const { return d->promptMessage; }
 ...
 */
QList<CellRange> DataValidation::ranges() const { return d->ranges; }
 ...
 */
void DataValidation::setValidationOperator(
 ...
 */
void DataValidation::setErrorMessage(const QString &error,
                                     const QString &title) {
 ...
 */
void DataValidation::setPromptMessage(const QString &prompt,
                                      const QString &title) {
 ...
 */
void DataValidation::setAllowBlank(bool enable) { d->allowBlank = enable; }
 ...
 */
void DataValidation::addRange(int firstRow, int firstCol, int lastRow,
                              int lastCol) {
...
bool DataValidation::saveToXml(QXmlStreamWriter &writer) const {
  ...
  static const QMap<DataValidation::ValidationOperator, QString> opMap = {
      ...
      {DataValidation::GreaterThanOrEqual,
       QStringLiteral("greaterThanOrEqual")}};
  ...
  if (validationOperator() != DataValidation::Between)
    writer.writeAttribute(QStringLiteral("operator"),
                          opMap[validationOperator()]);
  if (allowBlank())
    writer.writeAttribute(QStringLiteral("allowBlank"), QStringLiteral("1"));
  //        if (dropDownVisible())
  //            writer.writeAttribute(QStringLiteral("showDropDown"),
  //            QStringLiteral("1"));
  if (isPromptMessageVisible())
    writer.writeAttribute(QStringLiteral("showInputMessage"),
                          QStringLiteral("1"));
  if (isErrorMessageVisible())
    writer.writeAttribute(QStringLiteral("showErrorMessage"),
                          QStringLiteral("1"));
...
DataValidation DataValidation::loadFromXml(QXmlStreamReader &reader) {
  ...
  if (attrs.hasAttribute(QLatin1String("type"))) {
    ...
    validation.setValidationType(
        it != typeMap.constEnd() ? it.value() : DataValidation::None);
  }
  if (attrs.hasAttribute(QLatin1String("errorStyle"))) {
    ...
    validation.setErrorStyle(it != esMap.constEnd() ? it.value()
                                                    : DataValidation::Stop);
  }
  if (attrs.hasAttribute(QLatin1String("operator"))) {
    ...
    validation.setValidationOperator(
        it != opMap.constEnd() ? it.value() : DataValidation::Between);
  }
