...
CellFormulaPrivate::CellFormulaPrivate(const QString &formula_,
  ...
  if (formula.startsWith(QLatin1String("=")))
    formula.remove(0, 1);
  else if (formula.startsWith(QLatin1String("{=")) &&
           formula.endsWith(QLatin1String("}")))
    formula = formula.mid(2, formula.length() - 3);
}

CellFormulaPrivate::CellFormulaPrivate(const CellFormulaPrivate &other)
    : QSharedData(other), formula(other.formula), type(other.type),
      reference(other.reference), ca(other.ca), si(other.si) {}
...
/*!
  \class CellFormula
  \inmodule QtXlsx
  \brief The CellFormula class provides a API that is used to handle the cell
  formula.
...
CellFormula::CellFormula(const char *formula, FormulaType type)
    : d(new CellFormulaPrivate(QString::fromLatin1(formula), CellRange(),
                               type)) {}
 ...
 */
CellFormula::CellFormula(const QString &formula, const CellRange &ref,
                         FormulaType type)
 ...
 */
QString CellFormula::formulaText() const { return d ? d->formula : QString(); }
 ...
 */
bool CellFormula::isValid() const { return d; }
...
bool CellFormula::loadFromXml(QXmlStreamReader &reader) {
  ...
  if (typeString == QLatin1String("array")) {
    ...
    d->type =
        NormalType; // Change: normal Type is not mentioned in the xml file!!!!!
                    // }}
  }
