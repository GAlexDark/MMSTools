 ...
 * \inmodule QtXlsx
 *
 * The conditional formatting can be applied to a single cell or ranges of
 * cells.
 */
...
bool ConditionalFormatting::addHighlightCellsRule(HighlightRuleType type,
  ...
  if (type >= Highlight_LessThan && type <= Highlight_NotBetween) {
    ...
    if (type == Highlight_ContainsText) {
      ...
      cfRule->attrs[XlsxCfRuleData::A_operator] =
          QStringLiteral("containsText");
    ...
    if (type == Highlight_AboveOrEqualAverage ||
        type == Highlight_BelowOrEqualAverage)
...
bool ConditionalFormatting::addHighlightCellsRule(HighlightRuleType type,
  ...
  if ((type >= Highlight_AboveAverage && type <= Highlight_BelowStdDev3) ||
    ...
    return addHighlightCellsRule(type, QString(), QString(), format,
                                 stopIfTrue);
  }
 ...
 * \overload
 *
 * Add a highlight rule with the given \a type, \a formula, \a format and \a
 * stopIfTrue. Return false if failed.
 */
bool ConditionalFormatting::addHighlightCellsRule(HighlightRuleType type,
...
bool ConditionalFormatting::addDataBarRule(const QColor &color,
                                           ...
                                           const QString &val2, bool showData,
                                           bool stopIfTrue) {
  auto cfRule = std::make_shared<XlsxCfRuleData>();
 ...
 */
bool ConditionalFormatting::addDataBarRule(const QColor &color, bool showData,
                                           bool stopIfTrue) {
  return addDataBarRule(color, VOT_Min, QStringLiteral("0"), VOT_Max,
                        QStringLiteral("0"), showData, stopIfTrue);
}

/*!
 * Add a colorScale rule with the given \a minColor, \a maxColor and \a
 * stopIfTrue. Return false if failed.
 */
...
/*!
 * Add a colorScale rule with the given \a minColor, \a midColor, \a maxColor
 * and \a stopIfTrue. Return false if failed.
 */
 ...
 */
QList<CellRange> ConditionalFormatting::ranges() const { return d->ranges; }
...
/*!
    \overload
    Add the cell(\a row, \a col) on which the conditional formatting will apply
   to.
 */
 ...
 */
void ConditionalFormatting::addRange(int firstRow, int firstCol, int lastRow,
                                     int lastCol) {
...
bool ConditionalFormattingPrivate::readCfRule(QXmlStreamReader &reader,
  ...
  if (attrs.hasAttribute(QLatin1String("type")))
    rule->attrs[XlsxCfRuleData::A_type] =
  ...
  if (attrs.hasAttribute(QLatin1String("operator")))
    rule->attrs[XlsxCfRuleData::A_operator] =
        attrs.value(QLatin1String("operator")).toString();

  if (attrs.hasAttribute(QLatin1String("text")))
    rule->attrs[XlsxCfRuleData::A_text] =
        attrs.value(QLatin1String("text")).toString();
  ...
  if (attrs.hasAttribute(QLatin1String("rank")))
    rule->attrs[XlsxCfRuleData::A_rank] =
        attrs.value(QLatin1String("rank")).toString();

  if (attrs.hasAttribute(QLatin1String("stdDev")))
    rule->attrs[XlsxCfRuleData::A_stdDev] =
        attrs.value(QLatin1String("stdDev")).toString();
...
bool ConditionalFormattingPrivate::readCfDataBar(QXmlStreamReader &reader,
                                                 XlsxCfRuleData *rule) {
...
bool ConditionalFormattingPrivate::readCfVo(QXmlStreamReader &reader,
                                            XlsxCfVoData &cfvo) {
...
bool ConditionalFormatting::loadFromXml(QXmlStreamReader &reader,
                                        Styles *styles) {
...
bool ConditionalFormatting::saveToXml(QXmlStreamWriter &writer) const {
  ...
  writer.writeAttribute(QStringLiteral("sqref"),
                        sqref.join(QLatin1String(" ")));
  ...
  for (int i = 0; i < d->cfRules.size(); ++i) {
    ...
    if (rule->attrs[XlsxCfRuleData::A_type] == QLatin1String("dataBar")) {
      ...
      d->writeCfVo(writer,
                   rule->attrs[XlsxCfRuleData::A_cfvo1].value<XlsxCfVoData>());
      d->writeCfVo(writer,
                   rule->attrs[XlsxCfRuleData::A_cfvo2].value<XlsxCfVoData>());
      rule->attrs[XlsxCfRuleData::A_color1].value<XlsxColor>().saveToXml(
          writer);
      writer.writeEndElement(); // dataBar
    } else if (rule->attrs[XlsxCfRuleData::A_type] ==
               QLatin1String("colorScale")) {
      ...
      rule->attrs[XlsxCfRuleData::A_color1].value<XlsxColor>().saveToXml(
          writer);
      rule->attrs[XlsxCfRuleData::A_color2].value<XlsxColor>().saveToXml(
          writer);
