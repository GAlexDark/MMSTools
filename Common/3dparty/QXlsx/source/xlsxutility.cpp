...
QString getRelFilePath(const QString &filePath) {
  ...
  ret = QString(filePath.left(idx) + QLatin1String("/_rels/") +
...
double datetimeToNumber(const QDateTime &dt, bool is1904) {
  ...
  QDateTime epoch(is1904 ? QDate(1904, 1, 1) : QDate(1899, 12, 31),
                  QTime(0, 0));
...
QString escapeSheetName(const QString &sheetName) {
  // Already escaped.
  Q_ASSERT(!sheetName.startsWith(QLatin1Char('\'')) &&
...
bool isSpaceReserveNeeded(const QString &s) {
  ...
  return !s.isEmpty() &&
         (spaces.contains(s.at(0)) || spaces.contains(s.at(s.length() - 1)));
}

/*
 * Convert shared formula for non-root cells.
 *
 * For example, if "B1:B10" have shared formula "=A1*A1", this function will
 * return "=A2*A2" for "B2" cell, "=A3*A3" for "B3" cell, etc.
 *
 * Note, the formula "=A1*A1" for B1 can also be written as "=RC[-1]*RC[-1]",
 * which is the same for all other cells. In other words, this formula is
 * shared.
 *
...
QString convertSharedFormula(const QString &rootFormula,
    ...
    if (inQuote) {
      ...
      if (ch == QLatin1Char('"')) {
        ...
        if (refState == AZ) {
          ...
          segments.append(
              std::make_pair(segment, refState == _09 ? refFlag : -1));
  ...
  for (const auto &p : segments) {
    ...
    if (p.second != -1 && p.second != 3) {
      ...
      int row = p.second & 0x02 ? oldRef.row()
                                : oldRef.row() - rootCell.row() + cell.row();
      int col = p.second & 0x01
                    ? oldRef.column()
                    : oldRef.column() - rootCell.column() + cell.column();
      result.append(
          CellReference(row, col).toString(p.second & 0x02, p.second & 0x01));
    } else {
