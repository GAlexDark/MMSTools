...
*/
CellReference::CellReference(const QString &cell) { init(cell); }
...
void CellReference::init(const QString &cell_str) {
  static thread_local QRegularExpression re(
      QStringLiteral("^\\$?([A-Z]{1,3})\\$?(\\d+)$"));
 ...
 */
bool CellReference::isValid() const { return _row > 0 && _column > 0; }
