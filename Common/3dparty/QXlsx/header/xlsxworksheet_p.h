...
struct XlsxHyperlinkData {
  ...
  XlsxHyperlinkData(LinkType linkType = External,
      ...
      : linkType(linkType), target(target), location(location),
        display(display), tooltip(tip) {}
...
struct XlsxSheetFormatProps {
  XlsxSheetFormatProps(
      int baseColWidth = 8, bool customHeight = false,
      double defaultColWidth =
          8.430f, // https://learn.microsoft.com/en-us/office/troubleshoot/excel/determine-column-widths
      double defaultRowHeight = 15, quint8 outlineLevelCol = 0,
      quint8 outlineLevelRow = 0, bool thickBottom = false,
      bool thickTop = false, bool zeroHeight = false)
      : baseColWidth(baseColWidth), customHeight(customHeight),
        defaultColWidth(defaultColWidth), defaultRowHeight(defaultRowHeight),
        outlineLevelCol(outlineLevelCol), outlineLevelRow(outlineLevelRow),
...
struct XlsxRowInfo {
  XlsxRowInfo(double height = 0, const Format &format = Format(),
              bool hidden = false)
      : customHeight(false), height(height), format(format), hidden(hidden),
...
struct XlsxColumnInfo {
  XlsxColumnInfo(int firstColumn, // = 0,
                 int lastColumn,  // = 1,
                 bool isSetWidth, double width = 0,
                 const Format &format = Format(), bool hidden = false)
      : width(width), format(format), firstColumn(firstColumn),
        lastColumn(lastColumn), outlineLevel(0), isSetWidth(isSetWidth),
        customWidth(false), hidden(hidden), collapsed(false) {}
...
public:
  int checkDimensions(int row, int col, bool ignore_row = false,
  ...
  QList<QSharedPointer<XlsxColumnInfo>> getColumnInfoList(int colFirst,
                                                          int colLast);
