...
class CConsoleOutput : public QObject {
  Q_OBJECT
public:
  explicit CConsoleOutput(QObject *parent = nullptr) : QObject{parent} {
#ifdef Q_OS_WIN32
  ...
  void outToConsole(const QString &msg) const {
    if (!msg.isEmpty()) {
...
public slots:
  void printToConsole(const QString &msg) const { outToConsole(msg); }
};
