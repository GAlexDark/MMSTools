...
int main(int argc, char *argv[]) {
  ...
  QCoreApplication::setApplicationVersion(
      QStringLiteral("%1 tag %2 %3")
          .arg(BUILD_VER, BUILD_GIT,
               elcUtils::getFormattedDateTime(BUILD_DATE)));
  QString appName(QCoreApplication::applicationName());
  QString appPath(a.applicationDirPath());

  CConsoleOutput consoleOut;
  QString description(
      QStringLiteral("MMS Event Log Conversion Utility Version %1\nCopyright "
                     "(C) 2023 Oleksii Gaienko, %3\n\n"));
  description.append(
  ...
  if (sa.isRunning()) {
    consoleOut.outToConsole(
        QLatin1String("The another copy of the Utility is still running."));
    return 1;
  }
  ...
  consoleOut.outToConsole(
      QLatin1String("MMS Event Log Conversion Utility starting..."));
  ...
  if (!CElcConsoleAppSettings::instance().init(appPath, iniFile, false)) {
    consoleOut.outToConsole(
        QLatin1String("The settings class cannot be initialized."));
    return 1;
  }
  ...
  if (dbName.isEmpty()) {
    ...
    settings.setMain(QLatin1String("SETTINGS"), QLatin1String("db_file_name"),
                     dbName);
    consoleOut.outToConsole(
        QLatin1String("Unable to get database file name.\nThe database file "
                      "will be created on the default path."));
  }
  ...
  if (!elcUtils::mkPath(dbPath, errorString)) {
    consoleOut.outToConsole(
        QStringLiteral("Cannot create folder: %1\nDetails: %2")
            .arg(dbPath, errorString));
    return 1;
  }
  ...
  if (runningMode == RunningMode::RUNNINGMODE_DEFAULT ||
      runningMode == RunningMode::RUNNINGMODE_CLEAN_DB) {
    ...
    if (comb1) {
      ...
      if (!CBasicDatabase::trunvateDB(dbName, errorString, count, tables,
                                      creationStrings)) {
        consoleOut.outToConsole(
            QLatin1String("Cannot clean database: %1").arg(errorString));
        return 1;
      }
  ...
  if (runningMode != RunningMode::RUNNINGMODE_REPORT_ONLY &&
      runningMode != RunningMode::RUNNINGMODE_CLEAN_DB) {
    QString internalIpFirstOctet = settings.getInternalIpStartOctet();
    if (internalIpFirstOctet.isEmpty()) {
      consoleOut.outToConsole(
          QLatin1String("Error in internal IP address mask. Please check it in "
                        "the config file."));
      return 1;
    }
    ...
    consoleOut.outToConsole(
        QLatin1String("Start reading and converting files..."));

    CMmsLogsThreadReader loader;
    bool retVal = QObject::connect(&loader, SIGNAL(sendMessage(QString)),
                                   &consoleOut, SLOT(printToConsole(QString)));
    Q_ASSERT_X(retVal, "connect", "connection is not established");

    quint16 logID = 1;
    bool hasHeaders = settings.isDataHasHeaders();
    retVal = loader.init(logID, dbName, hasHeaders, internalIpFirstOctet,
                         &pragmaList);
    if (retVal) {
    ...
    QObject::disconnect(&loader, SIGNAL(sendMessage(QString)), &consoleOut,
                        SLOT(printToConsole(QString)));
    pragmaList.clear();

    if (retVal) {
      consoleOut.outToConsole(QLatin1String("Reading file(s) completed"));
    } else {
      consoleOut.outToConsole(QStringLiteral("Error reading file(s): %1")
  ...
  if (runningMode != RunningMode::RUNNINGMODE_IMPORT_ONLY &&
      runningMode != RunningMode::RUNNINGMODE_CLEAN_DB) {
    ...
    quint16 logID = 1;
    bool retVal = report.init(logID, dbName, reportName, &excludedUsers,
    ...
    if (retVal) {
      consoleOut.outToConsole(
          QStringLiteral(
              "Report generating finished.\nThe result in the %1 file.")
              .arg(reportName));
    } else {
      consoleOut.outToConsole(QStringLiteral("Error generate report: %1")
